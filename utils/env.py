...
class GymEnv(gym.Env):
    def __init__(self, env_name, seed, max_episode_length, action_repeat):
        ...
        self._max_episode_steps = self.max_episode_length
        ...
        print(f"[{self.robot_name}-{self.task_name}] robot size: {self.robot_size}, hazard coefficient: {self.h_coeff}")
...
def Env(env_name, seed=0, max_episode_length=1000, action_repeat=1):
    if 'laikago' in env_name.lower() or 'cassie' in env_name.lower() or 'mitcheetah' in env_name.lower():
